...
const Updateorder = () => {
    ...
    const [contact_number, setContactNumber] = useState(data.contact_number)
    const [delivery_address, setDeliveryAddress] = useState(data.delivery_address)
    const [food_item, setFoodItem] = useState(data.food_item)
    const [payment_method, setPaymentMethod] = useState(data.payment_method)
    const [total_amount, setTotalAmount] = useState(data.total_amount)
    const [checkBoxes, setCheckBoxes] = useState({
        checkbox1: food_item.includes("Pizza"),
        checkbox2: food_item.includes("Burger"),
        checkbox3: food_item.includes("Sandwich")
    });
    ...
    const handleSubmit = async (event) => {
        ...
        if (Object.keys(validationErrors).length === 0) {
            try {
                const response = await service.updateOrder(data._id, name, contact_number, delivery_address, food_item, payment_method, total_amount)
    ...
    return (
        <div className='updateorder-back'>
            <ToastContainer />
            <div className='updateorder-form py-4'>
                <form className='px-3'>
                    <div className='text-center'>
                        <h1 className="m-0 text-white">Update Your Order</h1>
                    </div>
                    ...
                    <div className='mt-4 text-center'>
                        <button type='submit' className='btn btn-warning btn-block rounded-5 w-75 fw-bold' onClick={handleSubmit}>Update Order</button>
                    </div>
                </form>
            </div>
        </div>
